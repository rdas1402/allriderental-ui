...
const PaymentGatewayPage = () => {
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      // Process payment with Razorpay - this handles both payment and verification
  ...
  const createConfirmedBooking = async (bookingData, paymentId) => {
    try {
      ...
      console.log("Creating confirmed booking via API...", bookingPayload);
      
      // Call API to create confirmed booking
      const response = await paymentAPI.processBookingPayment({
      ...
      if (response && response.success) {
        const realBookingId = response.booking?.id || response.bookingId;
        
        // Store booking in localStorage
        const bookingForStorage = {
          ...bookingPayload,
          id: realBookingId || bookingData.id,
          apiId: realBookingId,
  ...
  const handleUPIPayment = async () => {
    ...
    try {
      ...
      // Use paymentAPI for UPI payment processing
      const response = await paymentAPI.processBookingPayment({
        bookingId: booking.id,
        amount: amount,
        paymentMethod: "upi",
        paymentDetails: {
          upiId: "allriderental@upi"
        }
      });

      if (response.success) {
        // For UPI, show instructions and wait for payment
  ...
  const handleQRPayment = async () => {
    ...
    try {
      ...
      // Use paymentAPI for QR payment processing
      const response = await paymentAPI.processBookingPayment({
        bookingId: booking.id,
        amount: amount,
        paymentMethod: "qr",
  ...
  const handleMockPayment = async () => {
    ...
    try {
      ...
      // Use mock payment endpoint for testing
      ...
      if (response.success) {
        const confirmedBooking = await createConfirmedBooking(booking, `mock_${Date.now()}`);
